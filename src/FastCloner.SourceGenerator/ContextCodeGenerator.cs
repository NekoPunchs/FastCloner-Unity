...
namespace FastCloner.SourceGenerator
{
    internal sealed class ContextCodeGenerator
    {
        ...
        private readonly Dictionary<string, string> _sharedMethodNames = new();
        private readonly HashSet<string> _sharedNeededHelpers = [ ];
        private Dictionary<string, bool> _needsState = new();

        public ContextCodeGenerator(SourceProductionContext ctx, ContextModel model)
        {
            _ctx = ctx;
            _model = model;
        }
        ...
        public string Generate()
        {
            ...
            {
                GenerateTypeCloner(_ctx,typeModel);
            }
        ...
        private void GenerateTypeCloner(SourceProductionContext ctx, TypeModel model)
        {
            var cloneCtx = new CloneGeneratorContext(ctx, model, _sharedMethodNames, _sharedNeededHelpers);
            cloneCtx.NeedsStateClass = true;   // Use shared state class logic
            cloneCtx.UseStaticMethods = false; // Use instance methods for context

            // Populate overrides from our analysis
            foreach (var kvp in _needsState)
            {
                cloneCtx.SetCircularReferenceOverride(kvp.Key, kvp.Value);
            }

            // Pre-register all registered types to map to "Clone" method
            foreach (var regType in _model.RegisteredTypes)
            {
                cloneCtx.RegisterExternalMethod(regType.FullyQualifiedName, "Clone");
                cloneCtx.RegisterImplicitType(regType);
            }
            ...
            var needsState = _needsState.ContainsKey(model.FullyQualifiedName) && _needsState[model.FullyQualifiedName];

            // Only generate NotNullIfNotNull attribute if it's available in the runtime (not from polyfill)
            var notNullAttr = CloneGeneratorContext.NotNullIfNotNullAttr(_model.CodeAnalysisAvailable);
            ...
            if (!needsState)
            {
                ...
                GenerateCloneBody(cloneCtx, typeName, false);
                _sb.Append(cloneCtx.Source.ToString());
                cloneCtx.Source.Clear();
            }
            else
            {
            ...
            if (needsState)
            {
                ...
                GenerateCloneBody(cloneCtx, typeName, true, "state");
                _sb.Append(cloneCtx.Source.ToString());
                cloneCtx.Source.Clear();
                _sb.AppendLine("        }");
                _sb.AppendLine();
            ...
            CollectionHelperGenerator.GenerateHelpers(cloneCtx);
        ...
        private void WriteClassCloneBody(CloneGeneratorContext ctx, string typeName, bool useState, string? stateVarName) =>
            ClassCloneBodyGenerator.WriteClassCloneBody(ctx, typeName, useState, stateVarName, false);
        ...
        private bool CheckCycleDFS(string u, Dictionary<string, HashSet<string>> adj, Dictionary<string, VisitStatus> status, Dictionary<string, bool> reachesCycle)
        {
            ...
            if (adj.TryGetValue(u, out var neighbors))
            {
                foreach (var v in neighbors)
                {
                    ...
                    {
                        status[v] = VisitStatus.Unvisited;
                    }
