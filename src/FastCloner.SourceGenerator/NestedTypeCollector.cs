            ...
            Dictionary<string, MemberModel> nestedTypes)
        {
            ...
            if (TypeAnalyzer.IsDictionaryType(type))
            {
                ...
                if (dictTypes.HasValue)
                {
                    ...
                    var model = new MemberModel("NestedHelper", // Dummy name
            ...
            else if (TypeAnalyzer.IsCollectionType(type))
            {
                ...
                if (elemType != null)
                {
                    ...
                    var elemClonInterface = TypeAnalyzer.HasClonableInterface(elemType);
                    ...
                    var collHasCount = kind == MemberTypeKind.Collection && TypeAnalyzer.CollectionHasCountProperty(type);
                    var collHasIndexer = kind == MemberTypeKind.Collection && TypeAnalyzer.CollectionHasIndexer(type);
                    var elemClonableExtClass = elemClon ? TypeAnalyzer.ComputeExtensionClassFqn(elemType) : null;

                    var model = new MemberModel("NestedHelper",
                                                type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat),
                                                false,
                                                true,
                                                false,
