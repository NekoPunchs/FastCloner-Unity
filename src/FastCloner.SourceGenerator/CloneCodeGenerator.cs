...
namespace FastCloner.SourceGenerator
{
    internal sealed class CloneCodeGenerator
    {
        ...
        private void WritePublicFastDeepCloneMethod(string typeName, string fullTypeName)
        {
            ...
            var notNullAttr = CloneGeneratorContext.NotNullIfNotNullAttr(_context.Model.CodeAnalysisAvailable && !isStruct);
            ...
            var hasInitOnlyWithCycles = _context.CanHaveCircularReferences && _context.Model.Members.Any(m => m.IsInitOnly);
            var structWithReadonlyRefs = _context.Model.IsStruct && _context.Model.Members.Any(m => m is { IsValueType: false, IsReadOnly: true });
        ...
        private static string GetSafeTypeName(string typeName) =>
            typeName.Replace('<', '_').Replace('>', '_').Replace(',', '_').Replace(' ', '_').Replace('.', '_').Replace('[', '_').Replace(']', '_').Replace('?', '_');
        ...
        private string GetTypeParametersString() => _context.Model.TypeParameters.Count == 0 ? string.Empty : $"<{string.Join(", ", _context.Model.TypeParameters)}>";
        ...
        private void WriteClassCloneBody(string typeName, string fullTypeName, bool useState, string? stateVarName = null) =>
            ClassCloneBodyGenerator.WriteClassCloneBody(_context, typeName, useState, stateVarName, true, "source");
        ...
        private void WriteClonerClass()
        {
            ...
            foreach (var usage in _usages)
            {
                ...
                var castTypeParam = typeParamsArray == null || typeParamsArray.Length == 0 ? "T" : typeParamsArray[0];
                ...
                {
                    var collectionModel = usage.CollectionModel.Value;
                    ...
                    var callArgs = needsState ? $"(({argType})(object)source, state)" : $"(({argType})(object)source)";
                ...
                else
                {
                    if (_context.TryGetImplicitTypeModel(argType, out var implicitModel))
                    {
                        ...
                        var needsState = implicitModel.NeedsStateTracking && _context.NeedsStateTracking;
                        var callArgs = needsState ? $"(({argType})(object)source, state)" : $"(({argType})(object)source)";
            ...
            var fallbackCastTypeParam = typeParamsArray == null || typeParamsArray.Length == 0 ? "T" : typeParamsArray[0];
